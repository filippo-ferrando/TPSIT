...
void randomSong(Canzone *songs,
                int n) { // prende il contenuto della struttura playlist e lo
                         // carica rndomizzato in un vettore, poi lo stampa
  int k, i;
  ...
  for (int k = 0; k < n; k++) {
    printf("%d. %s  %s\n", (songs + *(vetRand + k))->numero,
           (songs + *(vetRand + k))->titolo, (songs + *(vetRand + k))->artista);
  }
